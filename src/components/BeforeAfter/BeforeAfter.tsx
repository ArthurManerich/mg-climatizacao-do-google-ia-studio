        ...
        {loading ? (
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <article className="overflow-hidden rounded-feature border border-white/10 bg-brand-navy-900 shadow-floating">
              <div ref={sliderContainerRef} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); updatePosition(event.clientX); }} onPointerMove={(event) => { if (event.currentTarget.hasPointerCapture(event.pointerId)) updatePosition(event.clientX); }} className="relative aspect-[4/3] w-full cursor-ew-resize select-none overflow-hidden bg-brand-navy-900 touch-pan-y sm:aspect-[16/10]">
                {afterFailed ? <UnavailableImage label="Imagem de depois" /> : <img data-comparison-image="after" src={currentItem.after_img} alt={`Resultado depois do serviço: ${currentItem.title}`} loading="lazy" decoding="async" referrerPolicy="no-referrer" draggable="false" onError={() => setAfterFailed(true)} className="absolute inset-0 h-full w-full object-cover" />}
                ...
                <div className="absolute inset-0 overflow-hidden" style={{ width: `${sliderPosition}%` }}>
                  {beforeFailed ? <UnavailableImage label="Imagem de antes" /> : <img data-comparison-image="before" src={currentItem.before_img} alt={`Situação antes do serviço: ${currentItem.title}`} loading="lazy" decoding="async" referrerPolicy="no-referrer" draggable="false" onError={() => setBeforeFailed(true)} className="absolute inset-0 max-w-none object-cover" style={{ width: containerWidth ? `${containerWidth}px` : '100%', height: '100%' }} />}
