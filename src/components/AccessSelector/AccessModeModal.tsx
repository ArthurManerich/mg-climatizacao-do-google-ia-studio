...
export default function AccessModeModal({ isOpen, onClose }: AccessModeModalProps) {
  ...
  return isOpen ? (
        <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-brand-navy-950/90 px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))] backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
          <m.div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="access-modal-title" tabIndex={-1} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.16 }} className="relative max-h-full w-full max-w-2xl overflow-y-auto rounded-feature border border-line bg-surface shadow-floating">
