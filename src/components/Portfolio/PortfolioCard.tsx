  ...
  return (
    <article id={`portfolio-item-${item.id}`} className="group overflow-hidden rounded-card border border-line bg-surface shadow-card transition-[border-color,box-shadow] hover:border-brand-cyan-600/40 hover:shadow-card-hover">
      <button type="button" onClick={(event) => onPreview?.(item, event.currentTarget)} className="relative block aspect-[4/3] min-h-44 w-full overflow-hidden bg-brand-navy-900 text-left" aria-label={`Ampliar foto do serviço: ${item.title}`}>
        {imageFailed ? <span aria-hidden="true" className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-slate-300"><ImageOff className="h-7 w-7 text-brand-cyan-400" /><span className="text-xs font-semibold">Imagem indisponível</span></span> : <img src={item.img} alt={item.title} aria-hidden="true" loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={() => setImageFailed(true)} className="h-full w-full object-cover transition-transform duration-base group-hover:scale-[1.02]" />}
        <span aria-hidden="true" className="absolute left-3 top-3 rounded-pill bg-brand-navy-950/85 px-3 py-1 text-[0.6875rem] font-bold uppercase tracking-wider text-white backdrop-blur-sm">{categoryLabel}</span>
        {!imageFailed && <span aria-hidden="true" className="absolute bottom-3 right-3 flex min-h-9 items-center gap-1.5 rounded-pill bg-brand-navy-950/85 px-3 py-1.5 text-xs font-bold text-white backdrop-blur-sm"><Maximize2 className="h-3.5 w-3.5 text-brand-cyan-400" /> Ampliar</span>}
      </button>
